    ...
    setItems(
      data.filter((element) => {
        ...
        return element.title
          .split(" ")
          .some((word) => word.toLowerCase().startsWith(query) && category);
      }),
    );
  }, [query, cardCategory]);
  ...
  return (
    <div className="flex flex-col justify-center items-center min-h-screen bg-ctp-base text-ctp-text overflow-hidden">
      <div className="absolute max-sm:inline-block flex gap-2 justify-between sm:mt-5 top-18 w-70 sm:w-90">
        <label
          forhtml="query"
          className="flex items-center bg-ctp-text rounded"
        >
          <input
            type="text"
            name="query"
          ...
          >
            <Icon
              ...
              className="size-10 p-2 bg-center text-ctp-base "
            />
          </button>
        </label>
        <select
      ...
      <div className="max-sm:ml-15 mt-30 sm:mt-20 ">
        {data.length > 0 ? (
          ...
          <p className="text-ctp-text text-center max-sm:-ml-15 text-lg font-poppins">
            Loading...
          </p>
        )}
      </div>
